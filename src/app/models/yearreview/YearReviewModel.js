...
const YearReviewModel = {
    fetchAllReviewYear: (callback) => {
        const query = `SELECT * FROM yearreview WHERE isDeleted = 0 ORDER BY yearName DESC`
    ...
    deleteYearToTrashAll: (id, UserId) => {
        return new Promise((resolve, reject) => {
            const query = `UPDATE yearreview SET isDeleted = 1 , deleteTime = CURRENT_TIMESTAMP , userDeleted = ${UserId} WHERE _id = ${id}`
    ...
    deleteYearAll: (id) => {
        return new Promise((resolve, reject) => {
            const query = `DELETE FROM yearreview WHERE _id = ${id}`
            connection.query(query, (err, results) => {
                if (err) {
                    return reject(err)
                }
