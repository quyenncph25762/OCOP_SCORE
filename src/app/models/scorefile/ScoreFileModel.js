...
`;
    ...
    getOne: (id, callback) => {
        const query = `SELECT scorefile .* WHERE IsDeleted = 0 AND _id = ?`
    ...
    create: (scoreFile, callback) => {
        const query = `INSERT INTO scorefile (RankOcop,ScoreTotal,ScoreTemp_id,ScoreDate,Employee_id,Product_id ,
            Customer_id,ScoreCommitee_id,CreatorUser_id,Note,Name,Code,IsActive,AttachFileName,AttachFileUrl
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
        const VALUES = [scoreFile.RankOcop, scoreFile.ScoreTotal, scoreFile.ScoreTemp_id, scoreFile.ScoreDate, scoreFile.Employee_id, scoreFile.Product_id, scoreFile.Customer_id, scoreFile.ScoreCommitee_id, scoreFile.CreatorUser_id, scoreFile.Note, scoreFile.Name, scoreFile.Code, scoreFile.IsActive, scoreFile.AttachFileName, scoreFile.AttachFileUrl]
